...
def main(args):
    ...
    if args.qbackend not in torch.backends.quantized.supported_engines:
        raise RuntimeError(
            "Quantized backend not supported: " + str(args.qbackend))
    ...
    if args.post_training_quantize:
        ...
        ds = torch.utils.data.Subset(dataset, indices=list(
            range(args.batch_size * args.num_calibration_batches)))
        ...
        if args.output_dir:
            ...
            if utils.is_main_process():
                torch.save(model.state_dict(), os.path.join(
                    args.output_dir, "quantized_post_train_model.pth"))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.inference_mode():
            ...
            evaluate(quantized_eval_model, criterion,
                     data_loader_test, device=torch.device("cpu"))
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--qbackend", default="fbgemm",
                        type=str, help="Quantized backend: fbgemm or qnnpack")
    ...
    parser.add_argument("--eval-batch-size", default=128,
